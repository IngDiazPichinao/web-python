...
/**
 *  SearchEngine taken from PicNet TableFilter
 *      ~lweberk (02.09.2011)
 */
 ...
 * @return {boolean}
 */
SearchEngine.prototype.doesTextMatchTokens = function (textToMatch, postFixTokens, exactMatch) {
 ...
 * @return {Array.<string>}
 */
SearchEngine.prototype.parseSearchTokens = function(text) {
    if (!text) { return null; }
 ...
 * @return {boolean}
 */
SearchEngine.prototype.doesNumberMatchToken = function(token, text) {
 ...
 * @return {number}
 */
SearchEngine.prototype.getNumberFrom = function(txt) {
    if (txt.charAt(0) === '$') {
        txt = txt.substring(1);
    }
 ...
 * @return {!Array.<string>}
 */
SearchEngine.prototype.normaliseExpression = function(text) {
 ...
 * @param {string} term
 */
SearchEngine.prototype.normaliseTerm = function(tokens, token, term) {
 ...
 * @return {!Array.<string>}
 */
SearchEngine.prototype.getTokensFromExpression = function(exp) {
 ...
 * @return {!Array.<string>}
 */
SearchEngine.prototype.allowFriendlySearchTerms = function(tokens) {
    var newTokens = [];
    var lastToken;
 ...
 * @return {string}
 */
SearchEngine.prototype.convertExpressionToPostFix = function(normalisedTokens) {
    var postFix = '';
    var stackOps = [];
    var stackOperator;
            ...
            if (stackOps.length === 0 || token === '(') {
                ...
                if (token === ')') {
                    ...
                    while (stackOps.length !== 0) {
                        ...
                        if (SearchEngine.EPrecedence[stackOps[stackOps.length - 1]] > SearchEngine.EPrecedence[token]) {
                            stackOperator = stackOps.pop();
                            postFix = postFix + '|' + stackOperator;
                        }
                        else { break; }
